...
export class PublicIntegrationsController {
  ...
  async uploadSimple(
    ...
    // The file is already in storage by now, so its real size is read back
    // and the stored copy dropped when it would overflow the allowance.
    const size = file.size || (await storedObjectSize(file.filename));
    try {
      await this._mediaService.assertStorage(org.id, size);
    } catch (err) {
      await deleteStoredObject(file.filename);
      throw err;
    }
