...
// How long a post that links to another post is willing to wait for that other
// post to publish before it gives up. A grace window rather than an instant
// failure, because the post it depends on can run late or be retried.
const DEPENDENCY_GRACE_MINUTES = 60;
const DEPENDENCY_POLL_INTERVAL = '1 minute';
...
export async function postWorkflowV106({
  ...
  // A post can embed `(post:<id>)` to link to another post's live URL, and that
  // URL only exists once the other post is out. Hold here until every reference
  // resolves; fail rather than publish a post with the link missing.
  const dependencyDeadline = dayjs().add(DEPENDENCY_GRACE_MINUTES, 'minute');

  while (true) {
    const dependencies = await postDependencies(organizationId, postId);
    if (dependencies.status === 'ready') {
      break;
    }

    const blocking = [...dependencies.dead, ...dependencies.pending];
    const outOfTime = !dayjs().isBefore(dependencyDeadline);

    if (dependencies.status === 'dead' || outOfTime) {
      const reason =
        dependencies.status === 'dead'
          ? `This post links to a post that will never publish (${blocking.join(
              ', '
            )})`
          : `This post links to a post that hasn't published yet (${blocking.join(
              ', '
            )})`;
      ...
      await inAppNotification(
        post.organizationId,
        `We couldn't post to ${post.integration?.providerIdentifier} for ${post?.integration?.name}`,
        `We couldn't post to ${post.integration?.providerIdentifier} for ${post?.integration?.name} because ${reason}. Nothing was published.`,
        true,
        false,
        'info'
      );
