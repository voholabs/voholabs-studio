...
The swap only happens with the user's approval. Before calling this tool: show the user the proposed replacement — mediaPreview turns its media-library path into the actual picture, in the conversation — and get their explicit yes. Only then call this. Replacing is not reversible from here: this tool has no undo, and the asset it removed comes back only if its path still exists in the media library and you run a second swap the other way. A call the user has not seen and approved is a call made too early.
