...
export class IntegrationRepository {
  ...
  async createOrUpdateIntegration(
    ...
    // `picture` is our own proxy URL, not the asset. It can only be built once
    // the row exists (the id is generated by the upsert), and it never changes
    // afterwards - which is the point: every consumer stores a URL that cannot
    // rot, while the image behind it is re-resolved from the network.
    const proxyUrl = IntegrationPictureService.proxyUrl(upsert.id);
    if (upsert.picture !== proxyUrl) {
      await this._integration.model.integration.update({
        where: { id: upsert.id },
        data: { picture: proxyUrl },
      });
      upsert.picture = proxyUrl;
    }
  ...
  // `url` here is what the network returned after we pushed a new avatar to it,
  // so it belongs in pictureSource like any other network URL. `picture` stays
  // the proxy URL - it is not the image, and it never changes.
