...
export class MediaService {
  ...
  async uploadFromUrl(org: string, url: string) {
    ...
    // Storage cap: refuse up front on the declared size, then again on the
    // bytes that actually arrived, since Content-Length may be absent or wrong
    try {
      await this.assertStorage(org, declaredSize);
    } catch (err) {
      await response.body.cancel();
      throw err;
    }
    ...
    try {
      await this.assertStorage(org, receivedBytes);
    } catch (err) {
      await this.storage.removeFile(uploaded.path).catch(() => {});
      throw err;
    }
  ...
  async saveUploadSessionFile(
    ...
    await this.checkUploadSession(org, sessionId);
    // The file is already in storage by now, so a refusal removes it again
    try {
      await this.assertStorage(org, fileSize || 0);
    } catch (err) {
      await this.storage.removeFile(filePath).catch(() => {});
      throw err;
    }
